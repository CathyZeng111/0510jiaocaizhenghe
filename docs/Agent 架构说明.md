...
跨教材整合不只需要识别“相同概念”，还需要发现“看似相同但定义口径不一致”的教学风险。系统在 `backend/app/graph_integration.py` 中加入 `detect_definition_conflicts()`：当两个节点具有相同规范化名称，且名称 token 重合度 `token_jaccard > 0.6`，但定义文本重合度 `definition_jaccard < 0.3` 时，会将其标记为知识冲突。冲突结果写入 `GraphIntegrationResult.conflicts`，同时挂到对应整合节点的 `metadata.conflicts`，前端节点详情面板会以红色警示块展示“定义不一致”的教材来源和相似度数据。

这个机制的价值在于，它把跨教材整合从“去重合并”推进到“教学审校”。例如不同教材可能都使用同一个术语，但一个教材强调形态学定义，另一个教材强调机制或临床范围；如果系统直接合并，教师可能误以为两者完全等价。冲突检测会把这类节点显式暴露出来，供教师通过多轮对话选择“保留”“拆分”或“合并并注明差异”。因此它既服务可解释整合，也服务后续的教师干预闭环。
